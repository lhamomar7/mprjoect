...
saveCart();
showToast(`تمت إضافة ${product.name} إلى السلة 🛍️`, 'gold');
}
...
const product = PRODUCTS.find(p => p.id === productId);
showToast(`تمت إضافة ${product?.name} إلى المفضلة ❤️`, 'gold');
}
...
itemsEl.innerHTML = cart.map(item => ` <div class="cart-item"> <div class="cart-item-img">${item.emoji}</div>
...
if (cart.length === 0) {
summaryEl.innerHTML = `       <div style="color:var(--gray);font-size:14px;text-align:center;padding:20px 0">
        السلة فارغة       </div>
    `;
} else {
summaryEl.innerHTML = cart.map(item => `       <div class="summary-item">         <span>${item.emoji} ${item.name} × ${item.qty}</span>         <span>₪${(item.price * item.qty).toLocaleString()}</span>       </div>
    `).join('');
}
...
shipping === 0 ? 'مجاني' : `₪${shipping}`;

document.getElementById('summaryTotal').textContent =
`₪${(getCartTotal() + shipping).toLocaleString()}`;
}
